 ...
 *  Change notes for v2 (02/26/2015): 
 *  - D3 projection bug - now link to d3js.org's version of the d3 projection (local version was not displaying points properly)
 *  - after updating the source CSV file, all AITOFF projections are now done within this javascript code using D3
 *  - updated the display of coordinate information in the tooltips (added galactic coordinates), and 
 *    display the currently active coordinates first
 *  - also updated the placement of tooltips depending on RA, Dec or l, b values 
 *  Special thanks to Arnaud Collioud for help in debugging the projection problem.
...
function draw(){
 ...
 var points = g.selectAll("circle").data(drawingData, function (d) { return d.id;})
	...
	.on("mouseover", function(d){
		  ...
		  div.html("<b>" + d.source + "</b><br/><br/><i>" + d.title + "</i><br/><br/>" + d.headline + "<br/><br/>" + "<img src=" + d.img + " alt='Image preview' width='150' height='150' /><br>Release Date: " + d.date + "<br/><br/><i>l: " + parseFloat(d.Xgal*-1).toFixed(2) + " b: " + parseFloat(d.Ygal).toFixed(2) + "<br><i>RA: " + parseFloat(d.Xeq*-1).toFixed(2)  + " Dec: " + parseFloat(d.Yeq).toFixed(2))
		      .style("left", function(x) {
			  var tipX;
			  if (d.Xgal*-1 < 180){ tipX = d3.event.pageX + 20;
			  } else { tipX = d3.event.pageX - 250; } 
			  return tipX + "px";})
		      .style("top", function(y) {
			  var tipY;
			  if (d.Ygal > 45) { tipY = d3.event.pageY - 20;
			  } else if (d.Ygal < -25 && currentZoom < 3.0) { tipY = d3.event.pageY - 240;
			  } else { tipY = d3.event.pageY - 95; }
			  return tipY + "px";})
		  
              } else if (coordinate==="Equatorial") {           
		  div.html("<b>" + d.source + "</b><br/><br/><i>" + d.title + "</i><br/><br/>" + d.headline + "<br/><br/>" + "<img src=" + d.img + " alt='Image preview' width='150' height='150' /><br><br>Release Date: " + d.date +"<br/><br/><i>RA: " + parseFloat(d.Xeq*-1).toFixed(2) + " Dec: " + parseFloat(d.Yeq).toFixed(2) +"<br/><i>l: " + parseFloat(d.Xgal*-1).toFixed(2) + " b: " + parseFloat(d.Ygal).toFixed(2)) 
		      .style("left", function(x) {
			  var tipX;
			  if (d.Xeq*-1 < 180){ tipX = d3.event.pageX + 20;
			  } else { tipX = d3.event.pageX - 250; } 
			  return tipX + "px";})
		      .style("top", function(y) {
			  var tipY;
			  if (d.Yeq > 45) { tipY = d3.event.pageY - 20;
			  } else if (d.Yeq < -25 && currentZoom < 3.0) { tipY = d3.event.pageY - 240;
			  } else { tipY = d3.event.pageY - 95; }
			  return tipY + "px";}) }
	  })
...
d3.select("#zplus").on('mouseover', function() {
    d3.select("#zplusimg").transition().duration(200).attr("opacity", 0.5); });
d3.select("#zplus").on('mouseout', function() {
    d3.select("#zplusimg").transition().duration(200).attr("opacity", 0.7); });
d3.select("#zmin").on('mouseover', function() {
    d3.select("#zminimg").transition().duration(200).attr("opacity", 0.5); });
d3.select("#zmin").on('mouseout', function() {
    d3.select("#zminimg").transition().duration(200).attr("opacity", 0.7); });
